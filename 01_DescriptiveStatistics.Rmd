...
d_SES %>%
  ...
  summarise(Both = mean(value2, na.rm = TRUE),
            .groups = "drop") %>% 
  mutate(SES = scale(Both, center = TRUE, scale = TRUE)[, 1]) -> df_SES
...
read_csv("united.csv", col_types = "cDcccdddddcddddddddlllld") %>% 
  ...
  summarise(Total_image = n(),
            ...
            Prop_FaceLook = Overlap_image/Both_image * 100) -> d_gaze
...
d_participant %>% 
  ...
  select(participant_name, Month, SES, MM, PropNonattuned, Total_comment, app_comment, Nonattuned_comment) %>% 
